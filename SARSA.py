...
class SARSA_Agent:
    def __init__(self, state_dim, action_dim, alpha=0.1, gamma=0.95, epsilon=0.1):
    ...
    def select_action(self, state):
        if np.random.rand() < self.epsilon:
            ...
            print(f'random action {action} chosen')
        else:
    ...
    def update(self, reward, next_state):
        ...
        if prev_state is not None and prev_action is not None: 
            self.next_action = agent.select_action(next_state)
            td_target = reward + self.gamma * self.Q[next_state][self.next_action]
            td_error = td_target - self.Q[prev_state][prev_action]
            self.Q[prev_state][prev_action] += self.alpha * td_error
